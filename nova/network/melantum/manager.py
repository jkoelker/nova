...
FLAGS = flags.FLAGS
FLAGS.register_opts(melantum_opts)
...
class MelantumManager(manager.SchedulerDependentManager):
    """
    NetworkManager class that communicates with Melange an Quantum
    """
    def __init__(self, *args, **kwargs):
        self.driver = importutils.import_module(FLAGS.network_driver)

        self.q_conn = quantum_connection.QuantumClientConnection()
        self.m_conn = melange.Connection()
        ...
        if FLAGS.network_global_uuid_label_map:
            ...
            LOG.debug(_log_kwargs('Network', map=self._nw_map))
        else:
            self._nw_map = {}

        super(MelantumManager, self).__init__(service_name='network',
                                              *args, **kwargs)
    ...
    def _clean_up_melange(self, tenant_id, instance_id,
        ...
        try:
            self.m_conn.allocate_for_instance_networks(tenant_id,
                    instance_id, [])
        except exception.MelangeConnectionFailed, e:
            LOG.exception(_("Error cleaning up melange"))
        ...
        if 'network_name' in network and network['network_name']:
            ...
            try:
                label = self.q_conn.get_network_name(network['tenant_id'],
                                                     network['network_id'])
                net['label'] = label
                # NOTE(jkoelker) Quantum can't be trusted to be called
                #                frequently and not timeout/thow 500's.
                #                Update the network_name field in melange
                #                so we have consistant access to the label
    ...
    def _update_melange_with_name(self, tenant_id, network_id, name):
        ...
        except exception.MelangeConnectionFailed:
        ...
        for m_vif in melange_vifs:
            ...
            if not network_name:
                ...
                try:
                    ...
                    # NOTE(jkoelker) This will leave leave network name null
                    #                in melange for. Hopefully Quantum will
                    #                respond next time. Fingers Crossed!
                    msg = _('Error get name for network_id %s')
                    LOG.exception(msg % network_id)
    ...
    #
    # NOTE(jkoelker) Accept **kwargs, for the backwards compat. Dont use
    #                them.
    @uuidize
    def allocate_for_instance(self, context, instance_id, rxtx_factor,
        ...
        # NOTE(jkoelker) Make sure we only request one allocation per network
    ...
    def get_instance_nw_info(self, context, instance_id, project_id,
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                ...
                LOG.exception(_log_kwargs(msg, instance_id=instance_id))
        return nw_info
    ...
    #                dhcp and multi_host setups. Melantum only supports
    #                injected.
