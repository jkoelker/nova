...
authorize = extensions.extension_authorizer('compute', 'melantum-networks')
...
class NetworkController(object):
    ...
    def create(self, req, body):
        ...
        try:
            network = self.network_api.create(context, label=label,
                                              cidr=cidr)
            QUOTAS.commit(context, reservation)
        except Exception:
...
class Melantum_networks(extensions.ExtensionDescriptor):
    """Melange + Quantum Network Management Extension"""

    name = "MelantumNetworks"
    alias = "melantum-networks"
    namespace = "http://docs.openstack.org/ext/services/api/v1.1"
    updated = "2012-03-07T09:46:43-05:00"

    def get_resources(self):
        ext = extensions.ResourceExtension('melantum-networks',
                                           NetworkController())
        return [ext]


def _sync_networks(context, project_id, session):
    # NOTE(jkoelker) Melantum only cares about the project_id
